...
> **Last counted 2026-09-18.** 87 of 88 items completed — the USER_FEEDBACK.md backlog is fully resolved (100%) and two of the three feature requests shipped: **11.1 CalendarView** (Temporal-API calendar grid: events, grouping, .ics import, connectivity contract, drag-to-edit) and **11.3 browser-API title badges with polyfill tooltips** (shared `browserApiRegistry` + `BrowserApiBadge` on 16 docs pages). The only open item is **11.2 per-component docs iconography (Iconify, one consistent iconset)**.
