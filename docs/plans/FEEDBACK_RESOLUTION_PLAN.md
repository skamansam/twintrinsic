...
| 1.9 | P3 | **Internationalization (i18n) support.** All doc pages need an LTR/RTL toggle. Switch from `text-left`/`text-right` to `text-start`/`text-end` Tailwind classes. Explore Paraglide integration for component-level translations. Target: Persian translation for demo pages. | ✅ **COMPLETE.** LTR/RTL toggle shipped (sweep of `text-start`/`text-end`, persisted `html[dir]`, `fa` locale, `/demo` pages fully translated). Full docs i18n sweep finished: **all 120 docs pages are i18n-enabled** (imports `m` from `$lib/paraglide/messages.js`) and `pnpm check:i18n` reports **0 untranslated prose, 0 backlog**. Final batches: the 12 remaining component pages (Timer, Lazy, LazyPanel, LocaleSwitcher, Masonry, TextInput, InvalidState, RadioGroup, FormField, FormBuilder, PropsTable + redirect stubs Card/InputSwitch/SelectGroup/Panel-LazyPanel), the 5 example pages (dashboard, dashboard/guide, game-map, shopping, shopping/guide), and the theming preview page — each with prose keys added to `messages/{en,es,fa}.json` and `m()` calls in markup. Shared `sec_*` heading keys reused throughout; page prose split into fragment keys around `<code>`/`<a>`/`<strong>` elements. The Panel/LazyPanel duplicate page was converted to a redirect stub pointing at Lazy/LazyPanel (1.8 convention). Persian locale-switch e2e coverage exists for previously-translated pages. **Convention:** code tokens containing braces stay in markup as `&#123;`/`&#125;` entities (ICU treats braces as placeholders). Paraglide output must be recompiled (`npx paraglide-js compile --project ./project.inlang --outdir ./src/lib/paraglide`) after adding keys — the output directory is gitignored. | Swept all 26 `text-left`/`text-right` usages to `text-start`/`text-end` (19 files); added a persisted LTR/RTL toggle to the docs layout (`html[dir]`); added a `fa` locale and rebuilt `/demo` + `/demo/paraglide` as fully translated pages (locale switcher, `dir` flipping via `getTextDirection`, pluralization, component-level `m()` usage via `DemoBadge`/`LocaleSwitcher`). Swapped `plugin-message-format@4` for `plugin-icu1` in `project.inlang/settings.json` because the old plugin cannot parse plurals. Docs-site chrome (nav/links/app name/component index) is translated; docs home (`/docs`), utilities (`/docs/utilities`), theming (`/docs/theming`), and completion (`/docs/completion`) prose now render through `m()` in en/es/fa, including the theme customizer's color/combination labels. Shared section keys (`sec_props`, `sec_accessibility`, `sec_keyboard`, `sec_slot`, …) are established for the rest of the sweep. **Per-component pages translated: 77 of 110** — Button, DataTable, the complete App group (App, AppHeader, BottomBar, Footer, Sidebar, ThemeToggle), the complete Basic group (Accordion, AccordionItem, Card, Container, Hero, Panel, Section, Separator, Splitter, Tooltip), the complete Navigation group (Breadcrumb, BreadcrumbItem, Menu, MenuItem, Tabs, TabList, Tab, TabPanel, TreeMenu), the complete Data Display group (Avatar, AvatarGroup, Badge, Carousel, CarouselItem, Chip, ChipGroup, CodeBlock, CodeBlockSpeed, CodeEditor, Map, Progress, Skeleton, Table + TableBody/TableCell/TableHead/TableHeader/TableRow, Tag, TagGroup, Timeline, TimelineItem, Tree, TreeNode), the complete Metrics group (AreaChart, BarChart, HorizontalBarChart, LineChart, PieChart, DonutChart, GaugeChart, ProgressMetric, KPICard, StatsCard, MetricGrid, MetricTrend, examples), and Form batches 1–2 (Input, Textarea, NumberInput, Select, Checkbox, Radio; Switch, Slider, Knob, Rating, Calendar, ColorPicker). Each translated page has a Persian locale-switch e2e test in `tests/e2e/DirToggle.test.js`. `pnpm check:i18n` reports 82 i18n-enabled docs pages (77 component pages + the component index + 4 chrome pages) and 38 still pending. Remaining: 33 component pages — Form batch 3 (FileUpload, FloatLabel, Form, ListInput, Listbox, Combobox, AutoComplete, Dropdown), the Feedback group (Modal, Toast, Stepper + sub-pages), and the Utility group + misc pages. Every batch = prose keys added to `messages/{en,es,fa}.json` + `m()` calls in the page + e2e coverage. **Convention:** code tokens that contain braces (e.g. `bg-{type}`, `on${string}`) stay in markup as `&#123;`/`&#125;` entities — ICU treats braces in a message as placeholders, and `{m.x()}` output is HTML-escaped, so entities cannot live inside a message. |
| 1.11 | P2 | **`llms.txt` drops prose from translated pages.** `scripts/build-llms-txt.mjs` extracts literal text nodes, so any page that renders prose through `m()` contributes only its code/demo markup to `static/llms.txt` — the human-readable sentences are missing. Options: resolve `m.*()` calls against `messages/en.json` during extraction, or skip translated pages from the file. | ✅ `resolveMessages()` in `scripts/build-llms-txt.mjs` substitutes every `m.key()` call with its base-locale string and decodes numeric HTML entities before the brace-stripper runs, so translated pages keep their prose in the digest. |
...
| 7.10 | P3 | **FormBuilder: new component (large scope).** Build forms from an OpenAPI spec or a simple array of field descriptors. Parse JSON data and generate forms. No external library needed. Should be built as a separate effort. | ✅ Implemented: `src/lib/components/Form/FormBuilder.svelte` generates forms from an OpenAPI Schema Object or a `fields` descriptor array. Maps schema types onto Twintrinsic controls (string→TextInput/Select, integer→NumberInput, boolean→Switch, array→ListInput, object→nested fieldset), resolves `$ref` (JSON-pointer, incl. nested), `allOf` (merged field sets), and `oneOf`/`anyOf` (first concrete variant) via the `components` prop. Full a11y wiring (`FormField` wrapper, `aria-required`, `aria-invalid`, `aria-live` errors). Unit + e2e tests, Storybook story, and fully translated docs page included. |
...
| # | Priority | Issue | Status |
|---|----------|-------|--------|
| 11.1 | P1 | **New: [CalendarView component built on the Temporal API](./CALENDARVIEW_DESIGN.md), with the existing Calendar renamed to CalendarInput.** Design note: `docs/plans/CALENDARVIEW_DESIGN.md`. The current date-picking Calendar becomes **CalendarInput** (rename component, `src/lib/index.ts` + `twintrinsic/components/CalendarInput` subpath export, stories/tests, docs route `docs/components/CalendarInput/CalendarInput`, redirect stub left at the old Calendar route per the 1.8 convention). The new **CalendarView** renders a regular calendar grid (month grid minimum; week/day views desirable) and supports: **(a) Events** — add events via a data-driven `events` prop and snippet/event-content slots; **(b) Temporal API foundation** — `Temporal.PlainDate` / `PlainDateTime` / `ZonedDateTime` for date math, time zones, and ranges (**native APIs only: no polyfill installed — the docs page carries a Browser support note so consumers can add `@js-temporal/polyfill` themselves if they target older browsers**); **(c) External calendar connectivity** — adapters for Google Calendar, Microsoft (Outlook/365), Apple (iCal/CalDAV), and other popular calendaring applications, with iCalendar (.ics) as the common-denominator sync format; **(d) Import** — accept Google Calendar exports (.ics/CSV), iCalendar (.ics), and other common formats; **(e) Event grouping (headline feature)** — a toggle that merges the same event appearing on multiple calendars into one visual event, with a badge showing the number of sources/people (dedup via iCal UID first, title+time fallback); **(f) Rich event display** — events can carry badges, icons, per-calendar colors, and status markers (recurring, cancelled, …), with snippet slots for full custom rendering; **drag-to-edit via the native HTML Drag and Drop API** (`draggable` events, cell `dragover`/`drop` highlight, `oneventmove` callback, keyboard-editing alternative — resizing out of scope). Must follow the standard component checklist: ARIA grid pattern with full keyboard navigation, WCAG 2.1 AA, restProps + `data-*`/`aria-*` index signatures, unit + e2e tests, Storybook story, docs page, i18n for all three locales. | ✅ **Done (2026-09-18).** All milestones complete: M1 CalendarInput rename · M2 month-grid MVP (ARIA grid, roving-tabindex keyboard nav, i18n, e2e) · M3 events (eventNormalize, chips with icon/badge/color/status, eventContent snippet, +N more popover) · M4 grouping (eventGroup uid→title+day+time fallback, source-count badge, color dots, grouping prop) · M5 .ics import (parseICal with Google+Outlook fixtures, recurring repeat marker) · M6 connectivity (connectCalendars contract: calendars prop + fetchEvents + oncalendarserror, calendarId stamping, color fallback, Google/Outlook/Apple recipes) · M7 drag-to-edit (dragEvents + oneventmove via native HTML DnD, keyboard alternative) · M8 close-out (badge e2e, grouping/drag/connectivity e2e, all gates green, completion page updated). Phase 2 follow-ups since shipped (2026-09-18): month/week/day views, RRULE expansion (`recurrence` prop + `expandRecurrences`), and `parseGoogleCsv` (Google CSV export import) — and CSS sub-row lanes (`eventLanes` helpers + `--lane-count` grid tracks) so overlapping multi-day events render side by side — **phase 2 fully closed**. Also fixed while integrating: parseICal now converts RFC 5545's exclusive all-day DTEND onto CalendarView's inclusive span convention (real Google .ics exports rendered one day too long). |
| 11.2 | P2 | **New: per-component iconography across the docs (Iconify).** Every component's docs page and its sidebar nav entry get an icon, sourced through [Iconify](https://iconify.design/) so we can pick from thousands of icons — but from **one consistent iconset** for the whole site (candidates: Lucide, Tabler, Material Symbols — choose during implementation and record the choice here). Mechanics: add Iconify support to the shared Icon component (or a docs-level wrapper) using **bundled** `@iconify-json/<set>` data rather than the runtime Iconify API, so builds stay offline/deterministic; define a single `componentIcons` map (component name → icon) shared by the docs h1 row and the TreeMenu sidebar; icons are decorative (`aria-hidden="true"`, `focusable="false"`) since the adjacent text carries the component name; icons are language-neutral so no message keys are needed. | ✅ **Done (2026-09-18).** Iconset chosen: **Tabler** (matches the library's existing `defaultIconset` and `tabler:` usage). Data is **bundled** via the `@iconify-json/tabler` dev dependency — `src/routes/docs/componentIcons.ts` registers it once with `@iconify/svelte`'s `addCollection` (docs-only module; no runtime API fetches, offline builds). `componentIcons` maps ~90 components; every name is verified against the bundled data by `tests/unit/componentIcons.test.ts` (a stale upstream rename fails CI). Icons render in every docs h1 row (flex-row sibling, `aria-hidden`, 11.3 badge pattern) and in the TreeMenu sidebar via its native `icon` field, enriched from the same map. |
| 11.3 | P1 | **New: browser-API badges beside every docs page title.** Each docs page h1 row shows badge(s) to the right naming the native browser API(s) the component is built on (CalendarInput → `<input type="date">`; CalendarView → **Temporal**; Tooltip → CSS Anchor Positioning + Popover; Modal/Toast → Popover; Switch → `:has()`; ThemeToggle → `light-dark()`; Accordion → `<details>`; ColorPicker → `<input type="color">`; FileUpload → `<input type="file">`; Carousel → CSS scroll snap/overflow; …). Every badge has a **tooltip** (the Tooltip component) stating that the feature is a native browser API and that the user **may need to install a polyfill for older browsers** — naming the exact package where one exists (`@js-temporal/polyfill` for Temporal, `@oddbird/css-anchor-positioning` for anchor positioning, …) and "no polyfill available" where none does. Implement via a shared registry (`{ label, mdnUrl, polyfill?, tooltipKey }`) rendered with Badge + Tooltip; tooltip strings go through Paraglide messages (en/es/fa); the registry doubles as the data source for the completion page's web-API checklist and generalizes the Browser-support note in `CALENDARVIEW_DESIGN.md` §1. | ✅ **Done.** Shared `BrowserApiBadge` component + `browserApiRegistry` ({label, mdnUrl, polyfill, noPolyfill}) in `src/lib/components/BrowserApiBadge/`; rendered beside 16 docs page titles (Map intentionally excluded — Leaflet is third-party, not native). Tooltips name the exact consumer polyfill (@js-temporal/polyfill, @oddbird/css-anchor-positioning) or state none exists; i18n en/es/fa; registry tests; completion-page checklist + utilities-page docs updated; e2e verified with badges as h1 *siblings* so heading text stays clean. |
| 11.4 | P2 | **New: Holiday-calendar playground demo (CalendarView phase-2 showcase).** A docs-page example that lets users pick from Google's **public holiday calendars** (calendarId list served as static data — `en.usa#holiday@group.v.calendar.google.com`, `en.uk#holiday@group.v.calendar.google.com`, `en.german#holiday@group.v.calendar.google.com`, `en.christian#holiday@group.v.calendar.google.com`, etc.; fetched server-side via the M6 `calendars` contract and cached, since browser CORS blocks direct calls) with **multi-select toggles** per calendar and per-source colors. Add a **sandbox event layer** on top: a button to **sprinkle random events** across the currently visible period (month/week/day — whatever view is active; configurable density; deterministic seed option for e2e stability), plus **full event management** — create custom events (title/date/color picker), edit via the drag-to-edit M7 path or a form, and delete (with an `oneventdelete`/management-API addition to CalendarView or composition in the demo). Events carry icons/badges per the M3 rich display. Includes Storybook story + docs section, unit/e2e tests with a seeded RNG, and i18n en/es/fa. | ✅ **Done (2026-09-18).** Playground on the CalendarView docs page: multi-select Google public holiday feeds (US/UK/German/Christian) via `googleHolidaySource` + a server proxy (`/api/holidays` — the M6 CORS recipe dogfooded; feeds are plain iCal parsed by `parseICal`, cached per month client-side and per feed server-side). Sandbox: seeded-RNG **sprinkle button** (mulberry32, deterministic, targets the *active view's* span), custom-event form (title+date), select-then-delete (holidays excluded), drag-to-edit with `eventsDraggable` locking all calendar-sourced chips. Views demo (month/week/day switcher) added alongside. Playground story verifies merge + drag-lock; 4 e2e tests incl. live-feed fetch. i18n en/es/fa. **Follow-up (2026-09-19): the playground is now also a standalone example page** at `/docs/examples/calendar` (visible in the docs menu under Examples), sharing its generators (`googleHolidaySource`, seeded `sprinkleEvents`, `viewSpan`) with the docs-page demo via `src/routes/docs/examples/playgroundHelpers.ts`; 5 e2e tests (sprinkle, custom form, live holiday feed + drag-lock, fa/RTL chrome); i18n keys in en/es/fa. |
...
| New Feature Requests | 0 | 2 | 1 | 0 | 3 |
| **Total** | **82** | **2** | **3** | **2** | **88** |

> **Last counted 2026-09-18.** **89 of 89 items completed** — the USER_FEEDBACK.md backlog is fully resolved (100%) and all four feature requests (11.1 CalendarView, 11.2 docs iconography, 11.3 browser-API badges, 11.4 holiday playground) have shipped.
...
1. **CalendarView component (11.1):**
  ...
  2. Build CalendarView month grid on native Temporal types (typed global via `temporal.d.ts`; **no polyfill installed** — consumer-facing Browser support note in the docs); add week/day views
...
9. **Docs iconography (11.2):** choose one Iconify iconset; bundle `@iconify-json/<set>` data; add per-component icons to docs h1 rows + TreeMenu sidebar via a shared `componentIcons` map
10. **Browser-API badges (11.3):** shared registry (label, MDN url, polyfill, tooltip key) rendered as Badge + Tooltip beside every docs h1; tooltips name the polyfill to install for older browsers; i18n en/es/fa
