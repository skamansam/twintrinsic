...
describe("Timer", () => {
  ...
  it("is externally controllable via the running prop", async () => {
    // running=false: the timer must not tick (live prop flips are covered by
    // the Toast pause/resume tests, which drive the same `running` input)
    render(Timer, { props: { duration: 60, running: false } });
    await vi.advanceTimersByTimeAsync(5_000);
    expect(screen.getByRole("timer")).toHaveTextContent("01:00");

    cleanup();

    render(Timer, { props: { duration: 60, running: true } });
    await vi.advanceTimersByTimeAsync(5_000);
    expect(screen.getByRole("timer")).toHaveTextContent("00:55");
  });

  it("does not keep firing oncomplete when controlled and complete", async () => {
    const oncomplete = vi.fn();
    render(Timer, { props: { duration: 2, running: true, oncomplete } });
    await vi.advanceTimersByTimeAsync(3_000);
    // Complete once; without a parent flip the timer must not loop
    expect(oncomplete).toHaveBeenCalledTimes(1);
  });

  it("hides the readout when showReadout is false", () => {
    const { container } = render(Timer, { props: { showReadout: false } });
