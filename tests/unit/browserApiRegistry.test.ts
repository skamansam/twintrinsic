...
import { browserApiRegistry, browserApisFor } from "../../src/lib/components/BrowserApiBadge/browserApiRegistry.js"

describe("browserApiRegistry", () => {
	it("maps CalendarView to the Temporal API with its consumer polyfill", () => {
		const apis = browserApisFor("CalendarView")
		expect(apis).toHaveLength(1)
		expect(apis?.[0].label).toBe("Temporal")
