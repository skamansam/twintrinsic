...
describe("normalizeEvent", () => {
  ...
  it("accepts PlainDate, PlainDateTime, and ZonedDateTime instants", () => {
    ...
    expect(normalizeEvent({ id: "a", title: "d", start: pd }).startDay.toString()).toBe(
      "2026-09-17",
    );
    expect(normalizeEvent({ id: "b", title: "dt", start: pdt }).startDay.toString()).toBe(
      "2026-09-17",
    );
    // 23:30 in LA is Sept 17 in the event's own zone — the design guarantee.
    expect(normalizeEvent({ id: "c", title: "zdt", start: zdt }).startDay.toString()).toBe(
      "2026-09-17",
    );
  });
  ...
  it("infers allDay=false for timed instants and honors explicit allDay", () => {
    ...
    expect(normalizeEvent({ id: "5", title: "t", start: "2026-09-17", allDay: false }).allDay).toBe(
      false,
    );
    expect(
      normalizeEvent({ id: "6", title: "t", start: "2026-09-17T08:00", allDay: true }).allDay,
    ).toBe(true);
  });

  it("expands multi-day spans inclusive of the end day", () => {
    const ne = normalizeEvent({
      id: "7",
      title: "Conference",
      start: "2026-09-15",
      end: "2026-09-18",
...
describe("eventsForDay", () => {
  ...
  it("returns events whose span covers the day (inclusive both ends)", () => {
    const ids = events()
      ...
      .map((e) => e.event.id);
    const covered = eventsForDay(events(), TemporalPolyfill.PlainDate.from("2026-09-17")).map(
      (e) => e.event.id,
    );
