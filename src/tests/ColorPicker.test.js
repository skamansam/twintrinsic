...
test.describe('ColorPicker Component', () => {
  ...
  test('handles lightness slider', async ({ page }) => {
    ...
    await expect(colorPicker.locator('.color-swatch'))
      .toHaveCSS('background-color', /rgba\(0,\s*0,\s*0,\s*1\)/);
    
    // Set to 100% (white)
    await lightnessSlider.fill('100');
    await expect(colorPicker.locator('.color-swatch'))
      .toHaveCSS('background-color', /rgba\(255,\s*255,\s*255,\s*1\)/);
  });
  ...
  test('handles alpha slider', async ({ page }) => {
    ...
    await expect(colorPicker.locator('input'))
      .toHaveValue(/rgba\(\d+,\s*\d+,\s*\d+,\s*0\.5\)/);
  });
  ...
  test('handles direct input', async ({ page }) => {
    ...
    await expect(colorPicker.locator('.color-swatch'))
      .toHaveCSS('background-color', /rgba\(255,\s*0,\s*0,\s*1\)/);
    
    // Test rgb input
    await input.fill('rgb(0, 255, 0)');
    await input.press('Enter');
    await expect(colorPicker.locator('.color-swatch'))
      .toHaveCSS('background-color', /rgba\(0,\s*255,\s*0,\s*1\)/);
  });
