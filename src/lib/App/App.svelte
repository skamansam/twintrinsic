...
<div class='app bg-element-100 dark:bg-dark dark:text-light min-h-screen' style="--left-panel-width: {leftPanelWidth}; --right-panel-width: {rightPanelWidth};">
  ...
  {/if}
  <div class="appWrapper w-full flex flex-col sm:flex-row flex-wrap sm:flex-nowrap py-4 flex-grow">
    {#if $$slots.leftPanel}
      <div class="appLeftPanel w-fixed w-full flex-shrink flex-grow-0 px-4">
          <div class="sticky top-0 p-4 w-full h-full">
              <slot name="leftPanel"/>
          </div>
      </div>
    {/if}
    <main role="main" class="appMain w-full flex-grow pt-1 px-3">
      <slot/>
    </main>
    {#if $$slots.rightPanel}
      <div class="appRightPanel w-fixed w-full flex-shrink flex-grow-0 px-2">
          <!-- fixed-width -->
